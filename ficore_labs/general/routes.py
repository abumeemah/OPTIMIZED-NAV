...
@limiter.limit("5000 per minute", exempt_when=exempt_crawlers)
def landing():
...
@general_bp.route('/language-demo')
def language_demo():
    """Demo page showcasing instant language switching."""
    lang = session.get('lang', 'en')
    try:
        return render_template(
            'general/language_demo.html',
